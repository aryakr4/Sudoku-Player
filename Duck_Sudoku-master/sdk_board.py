...
class Board(object):
    ...
    def __init__(self):
        ...
        for cols in range(len(self.tiles[0])):
            new_list = []
    ...
    def min_choice_tile(self) -> Tile:
        ...
        for group in self.groups:
            ...
            for tile in un_tiles:
                if len(tile.candidates) < most:
                    most = len(tile.candidates)
                    lowest = tile  # set equal to the tile with the least/lowest candidates
        return lowest
    ...
    def solve(self):
        ...
        if self.is_consistent() is False:
            return False
        li = self.as_list()  # representation
        low_tile = self.min_choice_tile()
        for can in low_tile.candidates:
            low_tile.set_value(can)
            if self.solve():
                return True
            else:
                self.set_tiles(li)
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            ...
            for tile in group:  # 2nd iteration to remove the symbols
                if tile.value == UNKNOWN:
                    tests = tile.remove_candidates(sym)
                    if tests == True:
                        f = True
        return f
            ...
            for val in leftovers:
                ...
                for tile in group:
                    if val in tile.candidates:
                        rep += 1
                if rep == 1:  # if value only shows up once after iteration through group
                    for tile in group:
                        if val in tile.candidates:
                            tile.set_value(val)
                    already_used = True
